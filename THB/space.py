...
class Space:
    ...
    def initialize_datastructures(self):
        self.knotvectors = {level:tuple(bs.knotvector for bs in TP.bsplines) for level, TP in self.H.items()}
        
        self.sh_knots = {level:tuple(len(np.unique(self.knotvectors[level][i])) for i in range(self.ndim)) for level in range(self.num_levels)}
        self.sh_cells = {level:tuple(len(np.unique(self.knotvectors[level][i]))-1 for i in range(self.ndim)) for level in range(self.num_levels)}
